<template>
	<view class="home">
		<Nav title="添加使用者" bg="#02CD88" :back="true" @back="onBack"/>
		...
		<view class="list">
			...
			<view class="item">
				...
				<view class="right">
					<input class="text" style="margin-right: 10px;color: #333333;" type="text" v-model="pat_name" placeholder="请填写" placeholder-style="color:#999999" @input="onNameKey" maxlength="10" />
				</view>
			</view>
...
<script lang="ts" setup>
	...
	import { getAge2 } from '@/assets/utils'
	import { getBirthDay } from '@/assets/utils'
	import { onLoad } from "@dcloudio/uni-app"
	import { usePatientStore, Patient } from '@/stores/patient';
	import { add } from '@/assets/patient';
	...
	onLoad((option:any) => {
		...
		beforePage = option?.from
		if( beforePage===pageFrom.users ){
			patient = Object.assign({}, option)
			patStore.modifingPatient( patient )
			pat_name.value = patient?.name
			pat_birth.value = patient?.birth
	...
	const onBack = ()=>{
		if( !beforePage ) uni.navigateBack()
		else if( beforePage===pageFrom.index ){
			uni.switchTab({
				url:'/pages/index/index'
			})
		}
		else if( beforePage===pageFrom.users ){
			uni.redirectTo({
	...
	const onSave = async ()=>{
		...
		const items: Patient[] = patStore.getPatients()
		console.log("已经存在的用户:", items, "当前用户:", patient)
		const existIndex = items?.findIndex(p=>p?.name===patient?.name)
		if( existIndex !==-1 ) {
			uni.showToast({ icon: 'none', title: '姓名不能重复,请尝试其他名字' })
			return
		}
